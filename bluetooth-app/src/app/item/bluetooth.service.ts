...
import { Characteristic, Peripheral, ReadResult, Service } from './bluetooth.common';
...
export class Util {
    ...
    public static strToDelimitedHexStr(str: string): string {
        let s = '';
        if (str) {
            const a: string[] = str.split('').map(c => this.toPaddedHexByte(c.charCodeAt(0)));
            s = '0x' + a.join(',0x');
        }
        return s;
    }
...
export class BluetoothService {
    ...
    addOrReplacePeripheral(peripheral: IPeripheral) {
        ...
        this.zone.run(() => {
            ...
            console.dir(this.peripheralList);
        });
    }
    ...
    findPeripheral(peripheralUUID: string): IPeripheral {
        const found = this.peripheralList.filter(periph => periph.UUID === peripheralUUID);
        if (found.length > 0) {
            return found[0];
        } else {
            return null;
    ...
    findService(peripheral: IPeripheral, serviceUUID: string): IService {
        if (peripheral.services) {
            const found = peripheral.services.filter(svc => svc.UUID.toLowerCase() === serviceUUID.toLowerCase());
            if (found.length > 0) {
                return found[0];
            }
        }
    ...
    findCharacteristic(service: IService, characteristicUUID: string): ICharacteristic {
        if (service.characteristics) {
            const found = service.characteristics.filter(char => char.UUID.toLowerCase() === characteristicUUID.toLowerCase());
            if (found.length > 0) {
                return found[0];
            }
        }
    ...
    scanForPeripherals(serviceUUIDs?: string[], seconds?: number): boolean {
        if (!serviceUUIDs) {
            serviceUUIDs = [];
        } // an empty array means scan for all services
        if (!seconds) {
            seconds = 3;
        } // plugin stops scanning after this many seconds
    ...
    private startScan(serviceUUIDs: string[], seconds: number): Observable<IPeripheral> {
        ...
        return fromEventPattern<IPeripheral>(
            ...
            function remove(handler: Function) {
                ...
                bluetooth.stopScanning().then(res => console.log('bluetooth scanning stopped'), err => self.attention('Problem with bluetooth stopScanning', err, true));
            }
        );
    }
    ...
    disconnectPeripheral(peripheralUUID: string) {
        ...
        if (!peripheral) {
            ...
            bluetooth.disconnect({ UUID: peripheralUUID }).then(res => console.log('peripheral disconnected'), err => this.attention('problem disconnecting peripheral', err, true));
        }
    }
    ...
    dialogWrite(characteristic: ICharacteristic, value?: any) {
        if (value) {
            ...
            dialogs
                .prompt({
                    message: 'Enter comma-delimited value(s) to write , e.g. 0x01 or 0x007F or 0x01,0x02 or 0x007F,0x006E',
    ...
    dialogWriteWithoutResponse(characteristic: ICharacteristic, value?: any) {
        if (value) {
            ...
            dialogs
                .prompt({
                    message: 'Enter comma-delimited value(s) to write (without response), e.g. 0x01 or 0x007F or 0x01,0x02 or 0x007F,0x006E',
                    okButtonText: 'Write it',
                    cancelButtonText: 'Cancel',
